...
class OptimalPirateAgent:
    def __init__(self, initial):
        ...
        self.V = {} # value function looks like this: {(state, 0): (best_action, best_value)}
    ...
    def generate_atomic_actions_result(self, state, actions_tuple):
        ...
        for action in actions_tuple:
            ...
            if action_type != "Terminate" and action_type != "reset":
                ship = action[1]
    ...
    def generate_all_possible_outcomes_from_action(self, state, action):
        """
    ...
    def generate_all_legal_outcomes(self, state):
        ...
        for action in all_actions:
            all_legal_outcomes.extend(self.generate_all_possible_outcomes_from_action(state, action))
    ...
    def reward(self, state, action):
        ...
        for act in action:
            ...
            # -----------------------------------------------------------------------------------------------
            # This is the old version of the reward function where we
            # would calculate the reward for encountering a marine in the next state and not the current state
            # -----------------------------------------------------------------------------------------------
    ...
    def value_iteration(self):
        ...
        """
        # The representation of the dynamic programming table V is such that:
        # V[(state, turns)] = (value, action) where value is the best value of the state and action is the best action
        # for the state in the turn t (turns)

        # initialize the value function for t = 0 so that V(s, 0) = 0 for all s:
        V = {}
        for state in self.all_possible_states:
            V[(state, 0)] = {"best_value": 0, "best_action": None}
        self.V = V

        all_possible_states = self.all_possible_states
        turns = 1  # we start from the first turn (t = 1) and we will update the value function for each turn
        while turns <= self.turns:

            # for each state we will update the value function
            for state in all_possible_states:

                state = eval(state)  # convert the string to a dictionary

                max_value = -float('inf')  # initialize the max value to be -inf

                best_action = None  # initialize the best action to be None

                # for each VALID action we will calculate the value of the state
                actions = self.actions(state)  # get all possible actions of the state
                ...
                for action in actions:
                    ...
                    for next_state in self.generate_all_possible_outcomes_from_action(state, action): #! check function
                        next_state = eval(next_state)

                        # P(s'|s, a)
                        p_s_prime_s_a = self.transition_probability(state, next_state)

                        # V(s')
                        V_s_prime = self.V[(str(next_state), turns - 1)]["best_value"]
                        # -----------------------For Debug-------------------------------
                        # key = (str(next_state), turns - 1)
                        ...
                        # sum(P(s'|s, a) * V(s'))
                        # ----------------------------------------------------------------
                        sum_of_transition_probabilities += (p_s_prime_s_a * V_s_prime)

                    # Q(s, a) = R(s, a) + sum(P(s'|s, a) * V(s'))
                    value_of_action = reward_of_state + sum_of_transition_probabilities

                    if value_of_action >= max_value:
                        max_value = value_of_action
                        best_action = action

                # V(s) = max(Q(s, a))  for all a possible actions
                self.V[(str(state), turns)] = {"best_value": max_value, "best_action": best_action}
